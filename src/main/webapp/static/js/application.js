...
function bootstrap() {
  let url = `http://localhost:8080/expense-reimbursment-system/api/resources`;
...
function logout() {
  ...
  window.location.href = `http://localhost:8080/expense-reimbursment-system/`;
}

function getUserAccounts(params, callback) {
  if (params == undefined) { params = "" }
  let url = `http://localhost:8080/expense-reimbursment-system/api/user-accounts?${params}`;
  let xhr = new XMLHttpRequest();
  xhr.open("GET", url);
  xhr.responseType = "json";
